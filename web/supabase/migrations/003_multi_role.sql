-- Allow a user to play multiple characters in the same play.
alter table user_plays
  alter column role type text[]
  using case when role is null then null else array[role] end;

-- Recreate get_play_cast to unnest the roles array so each character
-- gets its own row (consistent with the previous interface).
